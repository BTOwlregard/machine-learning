...
### Project Design
We must begin with some data preprocessing. Categorical variables (e.g. MARRIAGE) will be one-hot encoded, as well as the PAY_X variables in part (see Dataset and Inputs). The dataset will be split into 3 subsets: build, validation, and test. 
We will then evaluate at least four modeling approaches (logistic regression, gradient boosted tree, random forest, and neural network) with hyperparamter selection using the validation set, and reported accuracy (see Evaluation Metrics) on the test set. This process of modeling and evaluation will give us our best-accuracy model. We will then impose some restrictions on our data according to a hypothetical business  and regulatory context (remove demographic variables and restrict usage of neural networks and trees beyond a given depth), and re-run our modeling and hyperparameter tuning exercise to produce a best-accuracy model subject to realistic constraints. We will evaluate the accuracy of the models tested, identify reasonable decision boundaries under different scenarios (e.g. high/low relative cost of false positives) , discuss the relative advantages of each model and suggest situations in which each may provide the best available classifier. 
